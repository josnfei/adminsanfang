	...
	<div class="tiwei">
		<div class=title>
			...
			<Breadcrumb separator=">">
				...
				<BreadcrumbItem>码头数据维护</BreadcrumbItem>
			</Breadcrumb>
		</div>
			<!--搜索-->
		<Form ref="formInline" inline :label-width="80">
			<Form-item label="码头名称">
				<Input v-model="formInline.码头名称" :maxlength="16" placeholder="请输入"></Input>
			</Form-item>
...
<script>
	import { matouThead } from 'common/js/table'
...
//				this._getuserlist(page);
			...
			submitsearch(page) {
				let search = {}
				if(this.formInline.码头名称) {
					search.码头名称 = likeStrSearch(this.formInline.码头名称);
				}
			...
			add() {
				...
				this.$router.push('home/depManagement/matou/matouweihu');
			},
			edit(dataObj) {
