	...
	<div class="tiwei">
		<div class=title>
			...
			<Breadcrumb separator=">">
				...
				<BreadcrumbItem>穿堤管道数据维护</BreadcrumbItem>
			</Breadcrumb>
		</div>
			<!--搜索-->
		<Form ref="formInline" inline :label-width="80">
			<Form-item label="穿堤管道名称">
				<Input v-model="formInline.穿堤管道名称" :maxlength="16" placeholder="请输入"></Input>
			</Form-item>
...
<script>
	import { ctgdThead } from 'common/js/table'
...
//				this._getuserlist(page);
			...
			submitsearch(page) {
				let search = {}
				if(this.formInline.穿堤管道名称) {
					search.穿堤管道名称 = likeStrSearch(this.formInline.穿堤管道名称);
				}
			...
			add() {
				...
				this.$router.push('home/depManagement/ctgd/chuantiguandaoweihu');
			},
			edit(dataObj) {
