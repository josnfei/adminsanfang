...
<script>
	...
	import { setAction, getAction, divisionArr, filterDepartment } from 'common/js/util'
	...
	export default {
		data() {
			return {
				...
				postObj: {
					'order': {_id: -1},
					"searchValue": { 'plan_category': '专题预案' }
				},
				selectLoading: false,
				autoCompleteData: [],
				departmentList: []
		...
		methods: {
			// 刷新
            refresh(){
                this.postObj.searchValue = {plan_category: "专题预案"}
                this.getList()
            },
			selected(value) {
				if(value){
			...
			search() {
				...
				this.searchVal.plan_category = '专题预案'
